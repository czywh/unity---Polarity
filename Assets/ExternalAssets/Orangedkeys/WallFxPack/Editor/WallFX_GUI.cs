...
public class WallFX_GUI : ShaderGUI
{
    ...
    void DoMain()
    {
        ...
        editor.ShaderProperty(_SpeedAnimatedMode, MakeLabel(_SpeedAnimatedMode, "Ring animation speed: 1 = one cycle per second, 0.2 = one cycle per 5 seconds"));
        ...
        // [Polarity] ring animation controls (URP shader): speed multiplier, looping, and the easing curve
        MaterialProperty _RingLoop = FindProperty("_RingLoop");
        editor.ShaderProperty(_RingLoop, MakeLabel(_RingLoop, "On: the ring animation repeats forever. Off: plays once (Ring Speed still sets how long it takes)"));

        MaterialProperty _RingCurve = FindProperty("_RingCurve");
        editor.ShaderProperty(_RingCurve, MakeLabel(_RingCurve, "Easing exponent. 0.2 = fast start then slow (original); 1 = linear; >1 = slow start then fast"));
